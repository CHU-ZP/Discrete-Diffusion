...
def _save_samples(
    ...
    *,
    ...
    if _is_image_dataset(cfg):
        ...
        samples = diffusion.sample(model, spatial_shape, batch_size=4, device=device)
        save_voxel_grid(samples.cpu(), sample_dir / f"generated_voxels_step_{step:06d}.png")
...
def _balanced_conditioning_labels(
    ...
    *,
    ...
    if num_labels <= 0:
        raise ValueError("Conditional image sampling requires dataset.num_labels > 0.")
